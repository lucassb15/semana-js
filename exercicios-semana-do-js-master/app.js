...
const imparArr = randomNumbers.filter(numbers => numbers % 2 != 0);
...
const numAbaixo = crazyNumbers.reduce((accumulator, numbersAbaixo) => {
  if (numbersAbaixo < 501) {
    return accumulator + 1
  }
  return accumulator
}, 0);
console.log('Numeros abaixo de 501: ' + numAbaixo);
console.log('');
console.log('');
console.log('');
console.log('Exer03');
...
const aoQuadrado = numbers.map(quadrado => Math.pow(quadrado, 2));
...
const nomesSeries = tvShows.map(nomes => nomes.name)
...
const listaProdutos = cart.reduce((accumulator, produto) => {
